...
@section('content')
    <div class="min-h-screen bg-slate-50 text-slate-800 flex flex-col font-sans">
        ...
        <header class="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-xs">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
                ...
                <nav class="hidden md:flex items-center space-x-6 lg:space-x-8">
                    ...
                    <a href="{{ route('website.ethics') }}" class="text-sm font-semibold {{ request()->routeIs('website.ethics') ? 'text-red-600' : 'text-slate-600 hover:text-red-600' }} transition">Etika Publikasi</a>
                    <a href="{{ route('website.indexing') }}" class="text-sm font-semibold {{ request()->routeIs('website.indexing') ? 'text-red-600' : 'text-slate-600 hover:text-red-600' }} transition">Pengindeksan</a>
                ...
                <div class="flex items-center space-x-4">
                    @auth
                        <a href="{{ route('submissions.create') ?? route('dashboard') }}" class="inline-flex items-center justify-center px-4 py-2 text-sm font-semibold rounded-xl text-white bg-red-600 hover:bg-red-700 transition shadow-sm gap-1.5">
                            <i class="ki-filled ki-cloud-change text-white"></i> Kirim Naskah (Submit)
                        </a>
                    @else
                        <a href="{{ route('login') }}" class="text-sm font-semibold text-slate-700 hover:text-red-600 transition">Masuk</a>
                        <a href="{{ route('register') }}" class="inline-flex items-center justify-center px-4 py-2 text-sm font-semibold rounded-xl text-white bg-red-600 hover:bg-red-700 transition shadow-sm gap-1.5">
                            <i class="ki-filled ki-cloud-change text-white"></i> Kirim Naskah
                        </a>
                    @endauth
                </div>
            </div>
        ...
        <!-- Footer -->
        <footer class="bg-slate-950 text-slate-300 border-t border-slate-800 mt-16">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                    <div class="space-y-4 md:col-span-2">
                        <div class="flex items-center gap-3">
                            <div class="h-9 w-9 rounded-xl bg-red-600 flex items-center justify-center text-white font-black text-xl">
                                I
                            </div>
                            <span class="text-lg font-extrabold text-white tracking-tight">Yayasan Satriabudi Dharma Setia (IGNITE)</span>
                        </div>
                    ...
                    <div>
                        ...
                        <ul class="space-y-2 text-sm">
                            ...
                            <li><a href="{{ route('website.ethics') }}" class="hover:text-white transition">Etika Publikasi</a></li>
                            <li><a href="{{ route('website.indexing') }}" class="hover:text-white transition">Informasi Pengindeksan</a></li>
